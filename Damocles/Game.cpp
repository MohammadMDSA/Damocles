...
void Game::OnWindowSizeChanged(int width, int height, DXGI_MODE_ROTATION rotation)
{
    if (!m_deviceResources->WindowSizeChanged(width, height, rotation))
        return;
    if (rotation == DXGI_MODE_ROTATION::DXGI_MODE_ROTATION_IDENTITY || rotation == DXGI_MODE_ROTATION::DXGI_MODE_ROTATION_ROTATE180 || rotation == DXGI_MODE_ROTATION::DXGI_MODE_ROTATION_UNSPECIFIED)
        m_camera.SetSize(width, height);
    else
        m_camera.SetSize(height, width);
    CreateWindowSizeDependentResources();
